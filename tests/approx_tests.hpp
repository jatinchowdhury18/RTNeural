...
template <typename T>
int fastSigmoidTest(T limit)
{
    using namespace RTNeural;
    constexpr int layerSize = 8;
    constexpr int nIter = 1; //100;
    constexpr T range = (T) 5;

    auto testSigmoid = [=] (auto forwardFunc, auto& test_outs)
    {
        std::default_random_engine generator;
        std::uniform_real_distribution<T> distribution(-range, range);
        ...
        for(int i = 0; i < nIter; ++i)
        {
            ...
            {
                test_ins[n] = distribution(generator);
                actual_outs[n] = (T)1.0 / ((T)1.0 + std::exp(-test_ins[n]));
            }
    ...
    T test_outs alignas(RTNEURAL_DEFAULT_ALIGNMENT) [layerSize];
    FastSigmoid<T> fastSigmoid { layerSize };
    std::cout << "Testing FastSigmoid for data type " << dtype << std::endl;
    result |= testSigmoid([&fastSigmoid, &test_outs] (const T (&test_ins)[layerSize])
        {
            fastSigmoid.forward(test_ins, test_outs);
        }, test_outs);

    FastSigmoidT<T, layerSize> fastSigmoidT {};
    std::cout << "Testing FastSigmoidT for data type " << dtype << std::endl;
#if RTNEURAL_USE_XSIMD
    result |= testSigmoid([&fastSigmoidT, &test_outs] (const T (&test_ins)[layerSize])
        {
            constexpr int layerSize = 8; // MSVC can't capture this in the lambda
            using b_type = xsimd::simd_type<T>;
            constexpr auto b_size = (int)b_type::size;
            ...
            fastSigmoidT.forward(test_ins_v);

            for(int i = 0; i < v_size; ++i)
                xsimd::store_aligned(test_outs + i * b_size, fastSigmoidT.outs[i]);
        }, test_outs);
#elif RTNEURAL_USE_EIGEN
    result |= testSigmoid([&fastSigmoidT, &test_outs] (const T (&test_ins)[layerSize])
        {
            ...
            for(int i = 0; i < layerSize; ++i)
                test_outs[i] = fastSigmoidT.outs(i);
        }, test_outs);
#else
    result |= testSigmoid([&fastSigmoidT] (const T (&test_ins)[layerSize])
        {
            fastSigmoidT.forward(test_ins);
        }, fastSigmoidT.outs);
#endif
...
int approximationTests()
{
    ...
    result |= fastSigmoidTest<float>(1.0e-4f);
    result |= fastSigmoidTest<double>(1.0e-4);
