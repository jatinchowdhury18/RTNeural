...
template <typename T> class LSTMLayer : public Layer<T> {
public:
  ...
  virtual inline void forward(const T *input, T *h) override {
    for (size_t i = 0; i < Layer<T>::out_size; ++i) {
      fVec[i] = vMult(fWeights.W[i], input, prod_in, Layer<T>::in_size) +
                vMult(fWeights.U[i], ht1, prod_out, Layer<T>::out_size);
      iVec[i] = vMult(iWeights.W[i], input, prod_in, Layer<T>::in_size) +
                vMult(iWeights.U[i], ht1, prod_out, Layer<T>::out_size);
      oVec[i] = vMult(oWeights.W[i], input, prod_in, Layer<T>::in_size) +
                vMult(oWeights.U[i], ht1, prod_out, Layer<T>::out_size);
      ctVec[i] = vMult(cWeights.W[i], input, prod_in, Layer<T>::in_size) +
                 vMult(cWeights.U[i], ht1, prod_out, Layer<T>::out_size);
    }
